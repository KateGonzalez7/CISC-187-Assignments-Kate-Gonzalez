...
#### The average- and worst-case time complexities of insertion sort are both described as $O(N^2)$, because the array is traversed through a second time for each iteration. In an average-case scenario, the number of operations is noticeably less than the latter. Nevertheless, its difference in behavior is not significant enough to be reduced to that of a lower-degree polynomial. Starting at $i = 1$ is crucial when implementing insertion sort considering that the algorithm assumes the first element is sorted. By proxy, any elements before $i$ when $i > 1$ must also be sorted for the algorithm to work. Although two implementations may both be expressed with the same time complexity, it is a generalized description that does not account for any intricacies. These intricacies come in the form of constants and multipliers. While not enough to change the nature of the algorithm's behavior, they may still significantly affect the actual number of operations. This is why improving algorithms and removing any possible filler is important.
